...
class HuffmanSuite extends munit.FunSuite:
  ...
  test("count size of times") {
    assertEquals(times(string2Chars("hello, world")).length, 9)
  }
  ...
  test("decode a very short text (0pts)") {
    new TestTrees:
      assertEquals(decode(t1, List(0, 1)), "ab".toList)

  }

  test("encode a very short text (0pts)") {
    new TestTrees:
      assertEquals(encode(t2)("ab".toList), List(0, 0, 0, 1, 1) )
